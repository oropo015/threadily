...
import { PresetHashtagsInput } from "./preset-hashtags-input"
...
export function ThreadGenerator() {
  ...
  // Save preset hashtags to localStorage when they change
  useEffect(() => {
    if (typeof window !== "undefined" && isMountedRef.current) {
      try {
        localStorage.setItem("threadify-preset-hashtags", presetHashtags)
      } catch (error) {
        console.error("Error saving preset hashtags to local storage:", error)
      }
    }
  }, [presetHashtags])
  ...
  // Format hashtags for appending to posts
  const formatPresetHashtags = useCallback((hashtags: string): string => {
  ...
  const generateThread = useCallback(() => {
    ...
    setTimeout(() => {
      try {
        ...
        // Add preset hashtags to each post if they exist
        if (presetHashtags.trim()) {
          const formattedHashtags = formatPresetHashtags(presetHashtags)

          // Check if adding hashtags would exceed character limits
          const postsWithHashtags = posts.map((post, index) => {
            const postWithHashtags = post + formattedHashtags

            // Only add hashtags if they don't exceed the character limit
            // For the last post, always try to add hashtags
            if (!exceedsCharacterLimit(postWithHashtags, platform) || index === posts.length - 1) {
  ...
  }, [text, platform, splitPoints, mediaItems, presetHashtags, formatPresetHashtags, t, toast])
            ...
            <CardContent className="pt-6">
              <Tabs defaultValue="editor" className="w-full">
                ...
                <TabsContent value="editor" className="space-y-4">
                  ...
                  {/* Preset Hashtags Input */}
                  <div className="mt-4 border-t pt-4 border-gray-100 dark:border-gray-800">
                    <PresetHashtagsInput value={presetHashtags} onChange={setPresetHashtags} maxLength={200} />
                  </div>
                ...
                <TabsContent value="find-replace" className="space-y-4">
                  <FindReplaceTool text={text || ""} onReplace={(newText) => setText(newText)} t={t} />
                </TabsContent>
              </Tabs>
            </CardContent>
